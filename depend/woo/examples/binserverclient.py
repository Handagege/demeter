...
def reqbinserver(host, port, req_str, log_id = 0, timeout_usecs = 150000) :
    address = (host, port)
...
def reqbinserver_cluster(cluster, server_idx, req_str,
        log_id = 0, timeout_usecs = 150000, retry_times = 2) :
...
if __name__ == "__main__":
    ...
    beg = time.time()
    resp_str = reqbinserver_cluster([{"host":host, "port":port}], 0, req_str, 8888888)
    end = time.time()
